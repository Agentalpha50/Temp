...
const MainSubject1 = () => {
    ...
    useEffect(() => {
        const fetchQuestions = async () => {
            let { data: questions, error } = await supabase
                .from('wild_life') // Update table name if needed
                .select('*');
    ...
    const handleSubmit = async () => {
        ...
        const { data, error } = await supabase
            .from('users')
            .select('sub1_current_score, sub1_high_score')
            .eq('name', userName)
            .single();
        ...
        const newHighScore = Math.max(score, data.sub1_high_score);

        const { error: updateError } = await supabase
            .from('users')
            .update({
                sub1_current_score: score,
                sub1_high_score: newHighScore
            })
            .eq('name', userName);
