...
const MainSubject2 = () => {
    ...
    useEffect(() => {
        const fetchQuestions = async () => {
            let { data: questions, error } = await supabase
                .from('conservation_geography') // Update table name if needed
                .select('*');
    ...
    const handleSubmit = async () => {
        ...
        const { data, error } = await supabase
            .from('users')
            .select('sub2_current_score, sub2_high_score')
            .eq('name', userName)
            .single();
        ...
        const newHighScore = Math.max(score, data.sub2_high_score);

        const { error: updateError } = await supabase
            .from('users')
            .update({
                sub2_current_score: score,
                sub2_high_score: newHighScore
            })
            .eq('name', userName);
